...
(function ($) {
    ...
    function sliderPortfolio($el = $(document)) {
        ...
        const initSlider = async function () {
            ...
            await this.querySelectorAll('.content-slider .slide-item').forEach(($item, $index) => {
                ...
                pagination.prev.push(`<div class="swiper-slide">
                            <div class="box-content w-100 d-flex align-items-center">
                                ...
                                <div class='box-title'><h6 class='sm-title-block'>${slideContent.querySelector('.title')?.innerText}</h6></div>
                            </div>
                        </div>`);
            ...
            if (nextCon)
                nextCon.innerHTML = `<div class="swiper-wrapper">${pagination.next.join(' ')}</div>`;
            const navContainer = this.querySelector('.prev-paginate');
            if (navContainer)
                navContainer.innerHTML = `<div class="swiper-wrapper">${pagination.prev.join(' ')}</div>`;
        ...
        const NavSwiper = function (container, swiper) {
            ...
            setTimeout(function () {
                gsap.to($(container).find('.bg-container'), {opacity: 1})
            }, 3000)
